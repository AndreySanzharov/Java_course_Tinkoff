...
public class Tasks {
    ...
    public Animal LongestName(List<Animal> animals){
        animals = animals.stream().sorted((Animal n1, Animal n2) -> Integer.compare(n2.name().length(), n1.name().length())).limit(1).toList();
    ...
    public Integer WeightFromKtoIlong(List<Animal> animals, Integer k, Integer i){
        return animals.stream().filter((animal) -> ((animal.age() >= k) && (animal.age() <= i))).mapToInt(Animal::weight).sum();
    }
    //16
    public List<Animal> SomeSorts(List<Animal> animals){
        return animals.stream().sorted(Comparator.comparing(Animal::type).thenComparing(Animal::sex).thenComparing(Animal::name)).toList();
    }
    //17
